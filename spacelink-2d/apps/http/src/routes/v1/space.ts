...
import { addEelementSchema, CreateSpaceSchema, deleteElementSchema } from "../../types";
import client from "@repo/db/client";

export const spaceRouter = Router();

spaceRouter.post("/element", userMiddleware, async (req , res) => {
    const parseData = addEelementSchema.safeParse(req.body);
    if(parseData.error){
    ...
    try {
        ...
        if( ( parseInt(space.height) < parseData.data.x) || parseInt(space.width) < parseData.data.y) {
            res.status(400).json({message: "element out of bound"})
            return;
        }

        const spaceElementsList = await client.spaceElements.findMany({
            where: {
                spaceId: space.id
            }
        })

        for (const element of spaceElementsList) {
            if(element.x == parseData.data.x || element.y == parseData.data.y){
                res.status(400).json({message: "Can't place element on top of others"})
                return;
            }
        }
